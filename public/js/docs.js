/**
 * LetSkillify Documentation Javascript File
 */
"use strict";
...
( function () {
	...
	var Docs = {
		init: function () {
			...
			// # Register events
			LetSkillify.$body.on( 'click', '.document-link', function ( e ) {
				self.open( e.currentTarget.getAttribute( 'href' ).slice( 1 ) );
				var mainContent = LetSkillify.byClass( 'main-content' );
		...
		open: function ( id ) {
			...
			if ( activeId && templates[ activeId ] ) {
				LetSkillify.byId( 'document-view' ).innerHTML = templates[ activeId ].text;
				LetSkillify.byId( 'document-title' ).textContent = templates[ activeId ].title;
				LetSkillify.byId( 'document-view' ).classList.remove( 'search-result' );
			}
			$( LetSkillify.byClass( 'document-link' ) ).parent( 'li' ).removeClass( 'show' );
		...
		search: function ( query ) {
			...
			LetSkillify.byId( 'document-title' ).textContent = 'Search Result';
			...
			LetSkillify.byId( 'document-view' ).innerHTML = finalResult ? finalResult : '<h5 class="text-center">Nothing Found</h5>';
			LetSkillify.byId( 'document-view' ).classList.add( 'search-result' );
		}
	}

	LetSkillify.docs = Docs;
	$( window ).on( 'LetSkillify_complete', function () {
		LetSkillify.docs && LetSkillify.docs.init();
	} );
} )();