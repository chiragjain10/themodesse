...
// Only initialize cart when authentication state changes from false to true
watch(() => auth.isAuthenticated, async (newValue, oldValue) => {
    if (newValue && !oldValue && auth.user?.id) {
        try {
            await cart.initialize();
        } catch (error) {
...
// Only initialize cart once when user is first loaded, not on every navigation
watch(() => auth.user, async (newUser, oldUser) => {
    // Only initialize if user changed from null/undefined to a valid user
...
.modesse-logo {
    width: 140px;
    height: auto;
    animation: logo-bounce 1.5s infinite alternate cubic-bezier(.77, 0, .18, 1), logo-shimmer 2.5s infinite linear;
